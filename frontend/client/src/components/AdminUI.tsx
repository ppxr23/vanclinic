...
export function Btn({ children, onClick, variant = 'primary', size = 'md', style, disabled }: { children: React.ReactNode; onClick?: () => void; variant?: 'primary'|'secondary'|'outline'|'danger'; size?: 'sm'|'md'; style?: React.CSSProperties; disabled?: boolean }) {
  const variants = {
  ...
  return (
    <button onClick={onClick} disabled={disabled} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontFamily: 'inherit', fontWeight: 600, cursor: disabled ? 'not-allowed' : 'pointer', opacity: disabled ? 0.5 : 1, ...variants[variant], ...sizes[size], ...style }}>
      {children}
    </button>
  );
}
